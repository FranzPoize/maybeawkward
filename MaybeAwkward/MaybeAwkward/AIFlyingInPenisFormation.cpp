...
void AIFlyingInPenisFormation::update(MessageReceiver &aMessageReceiver,float dt)
{
	...
	aMessageReceiver.receiveMessage(std::make_shared<MoveMessage>(SINUS_STYLE_MOVEMENT_SPEED*dt,SINUS_STYLE_MOVEMENT_SPEED*cos(2*3.14f*mTimeFromStart/2000)*dt,false));
}